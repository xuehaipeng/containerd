...
impl DirectRestoreEngine {
    ...
    fn process_file(&self, backup_file_path: &Path, backup_root: &Path, result: &mut DirectRestoreResult) -> Result<()> {
        // Map backup file path to container target path
        let target_path = match self.map_backup_to_container_path(backup_file_path, backup_root) {
            Ok(path) => path,
            Err(e) => {
                ...
                result.failed_files += 1;
                result.failed_details.push(FailedFile {
                    path: backup_file_path.to_path_buf(),
                    error: format!("Path mapping failed: {}", e),
                });
                return Ok(());
            }
        };
        ...
        match copy_result {
            CopyResult::Success => {
                ...
                // Clean up successfully restored file from backup directory
                // Only clean files that were successfully restored and validated
                if !self.dry_run {
                    // First validate that the file was successfully restored
                    match self.validate_file_before_cleanup(backup_file_path, &target_path) {
                        Ok(()) => {
                            // File validation passed, proceed with cleanup
                            match self.cleanup_backup_file(backup_file_path) {
                                Ok(()) => {
                                    result.cleaned_files += 1;
                                    result.cleaned_details.push(backup_file_path.to_path_buf());
                                    info!("Cleaned backup file after successful restore and validation: {}", backup_file_path.display());
                                }
                                Err(e) => {
                                    warn!("Cleanup operation failed for {}: {}", backup_file_path.display(), e);
                                    // Don't fail the restore operation for cleanup failures
                                    // The file was successfully restored, cleanup failure is not critical
                                }
                            }
                        }
                        Err(e) => {
                            warn!("File validation failed before cleanup for {}: {}", backup_file_path.display(), e);
                            warn!("Preserving backup file due to validation failure: {}", backup_file_path.display());
                            // Don't attempt cleanup if validation fails - preserve the backup file
                        }
                    }
                } else {
                    info!("DRY RUN: Would validate and clean backup file after successful restore: {}", backup_file_path.display());
                }
            }
            CopyResult::Skipped(reason) => {
                result.skipped_files += 1;
                ...
                info!("Skipped file: {} - {} (preserving in backup for manual recovery)", target_path.display(), reason);
                // Explicitly preserve skipped files in backup directory for potential manual recovery
                debug!("Backup file preserved for manual recovery: {}", backup_file_path.display());
            }
            CopyResult::Failed(error) => {
                result.failed_files += 1;
                result.failed_details.push(FailedFile {
                ...
                error!("Failed to restore file: {} - {} (preserving in backup for manual recovery)", target_path.display(), error);
                // Explicitly preserve failed files in backup directory for potential manual recovery
                debug!("Backup file preserved for manual recovery: {}", backup_file_path.display());
            }
        }
