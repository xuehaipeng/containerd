...
pub fn transfer_data_with_mount_bypass(source: &Path, target: &Path, timeout: u64, bypass_mounts: bool) -> Result<TransferResult> {
    ...
    if bypass_mounts {
        info!("Mount bypass enabled - using optimized parallel copy with device ID detection");
        // Use our new optimized fast_copy with built-in device ID-based mount detection
        transfer_data_ultra_fast(source, target, timeout)
    } else {
